...
const MainMenuBar = () => {
    ...
    const toggleSubMenu = (itemId) => {
        setSubMenuOpen((prevIsSubMenuOpen) => (prevIsSubMenuOpen === itemId ? !prevIsSubMenuOpen : itemId));
    };
    ...
    const renderMenuItems = (items) => {
        return items.map((item) => (
            <li key={item.id} className={`nav-item dropdown ${isSubMenuOpen === item.id ? 'show' : ''}`}>
                <Link
                    className="nav-link"
                    to={item.url}
